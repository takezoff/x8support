...
- <img src="imgs/icons/ui_icon_80.png" width="18"> **プロジェクト新規作成**ボタンを押すと、空のプロジェクトを作成してStudioに遷移します。
- **フォルダタブ**はファイルリストを表示するフォルダを選択します。
    - <img src="imgs/icons/ui_icon_76.png" width="18"> **サンプルフォルダ**はアプリに同梱されているサンプルファイルが入っているフォルダです。ファイルをセーブしたり削除したりすることは出来ません。サンプルファイルのカスタマイズは可能ですが、セーブはプロジェクトフォルダで行われます。
...
Lua言語でプログラミングが出来ます。x8独自APIとLuaの標準ライブラリ（一部を除く）が利用出来ます。
