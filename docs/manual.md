...
**スクリーンキーボード**は外付けキーボードを使わずに文字入力を行う場合に利用します。`Shift`、`Ctrl`、`Alt`の修飾トグルには、その左にロックするトグルがあり、ロックされていないと１回の文字入力で修飾がオフになります。`Mode`キーで、**通常文字** → **特殊文字** → **カナ**、の順に入力モードが切り替わります。（カナはJPキーボードの場合だけです）
...
- **外付けキーボード**については、QWERTY配列の日本語キーボードと英語キーボードに対応しています。ただし、実際に動作確認されているキーボードは以下の通りです。
    - ロジクール K380 マルチデバイス Bluetooth キーボード（日本語配列）
    - Anker Ultra-Slim Bluetooth Keyboard (A7726)
- 接続されているキーボードタイプは自動的には判別されないため、**オプションメニュー** → **入力設定** → **外付けキーボード**で、接続されているキーボードタイプを設定する必要があります。
    - **QWERTY JP**でQWERTY配列の日本語キーボードとして認識します。**カナ**が入力出来ます。
    - **QWERTY US**でQWERTY配列の英語キーボードとして認識します。**カナ**は入力出来ません。

### **iOSでの日本語キーボード利用に関する注意事項**

- iOSで外付けの日本語キーボードを利用する場合、英語キーボードに存在しないキー（`¥`,`無変換`,`変換`,`かな`などのキー）は入力を取得出来ません。そのためプログラミングに必要な以下の文字入力については、別のキーを代用して入力します。
    - `\`（バックスラッシュ）は`漢字`キーで入力
