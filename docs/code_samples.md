...
アニメーションの仕組みは[ここで紹介したもの](#画像を切り替えてアニメーションしたい)と同じです。
...
アニメーションの仕組みは[ここで紹介したもの](#パターン毎にオフセット位置とサイズを指定してアニメーションしたい)と同じです。
...
[ここで紹介した](#パターン毎にオフセット位置とサイズを指定してアニメーションしたい)描画処理に反転処理を追加したものです。表示位置も反転を考慮した位置になります。
...
上記のコードを実行するには[ここで紹介した](#キャラクター情報をテーブルにして表示を管理したい)関数`newChar`、`updateCharAnim`、`drawChar`が必要です。
...
`isHit`は[ここで紹介した](#矩形どうしのヒットチェックをしたい)矩形どうしのヒットチェックを行う関数です。
...
[x8.map](reference.md##x8.map)を使って、Map領域のマップを描画することができます。
